...
<script type="text/javascript">
	var socLi =  document.getElementsByTagName("li");
	var content = document.getElementsByClassName("content");
	//alert(content);
	var socDiv = content[0].getElementsByTagName("div");
	//alert(socLi.length);
	//把所有的内容隐藏
	function hide()
	{
		for(var i=0; i<socDiv.length; i++)
		{
			socDiv[i].style.display = "none"; 
		}
	}
	function show(i)
	{
		hide();
		socDiv[i].style.display = "block"; 
	}
	hide();
	//点击新闻标题显示新闻
	for (var k = 0; k < socLi.length; k++)
	{
		socLi[k].index = k;	//添加用户自定义属性index储存每个li的下标
		socLi[k].onclick = function()
		{
			hide();
			socDiv[this.index].style.display = "block";
		}
	}
</script>
</body>
